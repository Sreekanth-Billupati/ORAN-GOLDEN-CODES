...
void Userplane_L1_Data_Gen(stream<l1_data_axis> &L1_data_out,ap_uint<4> &st_out, ap_uint<6> &symbol_number){
...
#pragma HLS INTERFACE ap_none port=st_out
 ...
 switch(data_src_state){
	case APPLICATION_HDR:{
		...
		data_src.payload.range(43,36) = FRAME_ID;
		data_src.payload.range(35,32) = SUBFRAME_ID;
		data_src.payload.range(31,26) = SLOT_ID;
...
case IDLE:
	    //sym_count=1;                      //LOOPING CONDITION - to be commented
